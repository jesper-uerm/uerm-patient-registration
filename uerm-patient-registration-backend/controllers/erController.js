...
const ErController = {
    ...
    try {
      ...
      if (!req.body.patientId) {
        return res.status(400).json({
          message: "Update failed: Patient Number (patientId) is missing.",
        });
      }
  ...
  fetchPatientRecords: async (req, res) => {
    try {
      ...
      const records = await ErModel.getPatientRecords(PATIENTNO);
  ...
  searchAdmittedPatients: async (req, res) => {
    try {
      const { query } = req.query; 
      let patients;

      if (query && query.trim() !== '') {
        patients = await ErModel.searchAdmittedPatients(query); 
      } else {
        patients = await ErModel.getAdmittedPatients();
      }
  ...
  generateCaseNumber: async (req, res) => {
    try {
      ...
      if (!data || Object.keys(data).length === 0) {
        return res
          .status(400)
          .json({ message: "Invalid Request: Payload is missing." });
      }

      if (!data.patientId) {
        return res
          .status(400)
          .json({ message: "Update failed: Patient ID is missing." });
      }
      ...
      if (!result) {
        return res.status(404).json({
          message: "Failed to update record. Patient ID may be invalid.",
        });
      }
